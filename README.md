...
1. System Initialization & Language Gate
...
a) Persistence Permanent storage in tickets.txt via fprintf

b) Protection Append mode ensures existing data remains safe

c) Logic Gate Language selection happens before the Main Menu loop

d) Input Safety Buffer clearing prevents crashes from non-numeric input

e) Exit Code9 breaks the while(1) loop for a clean shutdown
